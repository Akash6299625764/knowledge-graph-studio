...
def _local(uri):
    ...
    if not uri:
        return ""
    # handle ElementTree's {namespace}localname format
    if uri.startswith("{"):
    ...
    for el in root.findall(f"{{{OWL}}}NamedIndividual"):
        ...
        for child in el:
            ...
            if res:
                ...
                if obj_id and obj_id not in classes:
    ...
    for el in root.findall(f"{{{RDF}}}Description"):
        ...
        for child in el:
            ...
            if res:
                ...
                # skip if target is a class, not an individual
                if o_id and o_id != s_id and o_id not in classes:
