...
def _is_rate_limit(e):
    """Check if exception is a Groq 429 rate limit error."""
    msg = str(e)
...
def extract_graph_ai(pdf_path):
    if not GROQ_API_KEY or GROQ_API_KEY == "your_groq_api_key_here":
        print("[AI extractor] No Groq key — using spaCy fallback")
    ...
    # all chunks failed for other reasons
    if errors == len(chunks):
        print("[AI extractor] All chunks failed — using spaCy fallback")
        return _spacy_fallback(pdf_path)
